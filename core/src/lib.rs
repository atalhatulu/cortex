...
const TANS_HIST2D_SIZE: usize = 256 * 257 * 4;
    ...
    F: FnMut(usize, usize),
{
    ...
    if _metadata.is_some_and(|metadata| metadata.len() > MAX_METADATA_SIZE) {
            ...
            .map(|chunk_in| {
                if tans {
                    if content::classify(chunk_in).is_already_compressed() {
                        ...
                        let mut final_out =
                            Vec::with_capacity(36 + 4 + comp_hist2d.len() + out.len());
    ...
    F: FnMut(usize, usize),
{
    ...
    let expected_chunks = if orig_len == 0 {
        0
    } else {
        (orig_len - 1) / block_size + 1
    };
    ...
    let (stage1_tx, stage1_rx) =
        crossbeam_channel::bounded::<(usize, std::io::Result<DecompressStage1>)>(channel_cap);
    let (stage2_tx, stage2_rx) =
        crossbeam_channel::bounded::<(usize, std::io::Result<Vec<u8>>)>(channel_cap);

    let crypto_info_cloned = crypto_info.clone();
    let reader_handle = std::thread::spawn(move || {
        let mut global_idx = 0;
        let mut chunks_read_total = 0;
        while global_idx < expected_chunks {
            let mut batch_comp_chunks = Vec::new();
            let mut batch_orig_sizes = Vec::new();

            for _ in 0..batch_size.min(expected_chunks - global_idx) {
                ...
                // CTXT may use up to roughly four bytes per input byte in a
                // pathological token stream. This bound prevents a forged
                // length prefix from allocating arbitrary memory while still
                // leaving generous headroom for all supported modes.
                let max_chunk_len = block_size.saturating_mul(6).saturating_add(2 * 1024 * 1024);
                if chunk_len > max_chunk_len {
                    let _ = stage1_tx.send((
                        global_idx,
                ...
                batch_comp_chunks
                    ...
                    .try_for_each(|((mut chunk, size), chunk_idx)| {
                        if let Some((key, base_nonce)) = crypto_ref {
                            chunk = match crypto::decrypt_chunk(
                                key,
                                base_nonce,
                                chunk_idx as u64,
                                &chunk,
                        ...
                        let res = if is_fast {
                            match zstd::decode_all(chunk.as_slice()) {
                                Ok(d) if d.len() == size => Ok(DecompressStage1::Fast(d)),
                                Ok(_) => Err(std::io::Error::new(
                                    std::io::ErrorKind::InvalidData,
                                    "Fast chunk size does not match archive header",
                                )),
                                ...
                                if is_stored {
                                    ...
                                    let comp_len =
                                        u32::from_le_bytes(chunk[ptr..ptr + 4].try_into().unwrap())
                                            as usize;
                                    ptr += 4;
                                    if comp_len > chunk.len().saturating_sub(ptr) {
                                        return stage1_tx
                                            .send((
                                                chunk_idx,
                                                Err(std::io::Error::new(
                                                    std::io::ErrorKind::InvalidData,
                                                    "tANS histogram exceeds chunk boundary",
                                                )),
                                            ))
                                            .map_err(|_| ());
                                    }
                                    ...
                                    ptr += comp_len;
                                    let raw_hist2d =
                                        match zstd::stream::read::Decoder::new(comp_hist2d)
                                            .and_then(|decoder| {
                                                let mut limited =
                                                    decoder.take((TANS_HIST2D_SIZE + 1) as u64);
                                                let mut out = Vec::with_capacity(TANS_HIST2D_SIZE);
                                                limited.read_to_end(&mut out)?;
                                                Ok(out)
                                            }) {
                                            Ok(d) if d.len() == TANS_HIST2D_SIZE => d,
                                            Ok(_) => {
                                                return stage1_tx
                                                    .send((
                                                        chunk_idx,
                                            ...
                                            Err(_) => {
                                                return stage1_tx
                                                    .send((
                                                        ...
                                                        Err(std::io::Error::new(
                                                            std::io::ErrorKind::InvalidData,
                                                            "Failed to decompress hist2d",
                                                        )),
                                                    ))
                                                    .map_err(|_| ())
                                            }
                                        };
                                    let max_tokens = size.saturating_mul(2).saturating_add(64);
                                    if num_tokens as usize > max_tokens || chunk.len() < ptr + 1028
                                    {
                                        return stage1_tx
                                            .send((
                                                chunk_idx,
                                    ...
                                    let res = crate::HIST2D_POOL.with(|pool| {
                                        ...
                                        for ctx in 0..256 {
                                            for sym in 0..257 {
                                                hist2d[ctx][sym] = u32::from_le_bytes(
                                                    raw_hist2d[rptr..rptr + 4].try_into().unwrap(),
                                                );
                                                rptr += 4;
                                            }
                                        }
                                        ...
                                        let mut hptr = ptr;
                                        for i in 0..257 {
                                            hist[i] = u32::from_le_bytes(
                                                chunk[hptr..hptr + 4].try_into().unwrap(),
                                            );
                                            hptr += 4;
                                        }
                            ...
                            if chunk.len() < (mtf::LANES * 4) + 4 {
                                ...
                                if is_stored {
                                    ...
                                    let max_tokens = size.saturating_mul(2).saturating_add(64);
                                    if num_tokens as usize > max_tokens {
                                        return stage1_tx
                                            .send((
                                                chunk_idx,
                                                Err(std::io::Error::new(
                                                    std::io::ErrorKind::InvalidData,
                                                    "Invalid range-coded token count",
                                                )),
                                            ))
                                            .map_err(|_| ());
                                    }
        ...
        // The container has no footer, therefore extra bytes are never valid:
        // accepting them makes a truncated/concatenated archive ambiguous.
        let mut trailing = [0u8; 1];
        if let Ok(n) = in_file.read(&mut trailing) {
            if n != 0 {
                let _ = stage1_tx.send((
                    global_idx,
                    Err(std::io::Error::new(
    ...
    if let Err(e) = reader_handle.join() {
        return Err(std::io::Error::new(
            ...
            format!("Reader thread panicked: {:?}", e),
        ));
    }
    if let Err(e) = stage2_handle.join() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::Other,
            format!("Stage2 thread panicked: {:?}", e),
        ));
    }

    if processed != orig_len || num_comp_chunks != expected_chunks {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            "Archive ended before all expected output was decoded",
        ));
    }
